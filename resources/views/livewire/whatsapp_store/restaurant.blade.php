<div class="row">
    ...
    <div class="col-xl-3 col-lg-4  mb-40">
        <div class="items-tabs collapse-mobile" id="filterMenu">
            <div style="display: flex;">
                <div class="position-relative mb-22" style="width: 100vw;">
                    ...
                    <input type="search" placeholder="{{ __('messages.whatsapp_stores_templates.search_items') }}" id="productSearch"
                        wire:model.live="search" class="form-control ps-45" />
                </div>
                <div class="w-25 mt-2 mb-3 d-flex justify-content-end hide-desktop" style="position: relative;top: -5px;" data-content="Show Filter" onclick="showFilter()" id="filterButtonNew">
                ...
                <div>
                    @foreach ($categories as $category)
                        <div class="form-check mb-2 position-relative">
                            ...
                            <i class="fa-solid fa-share-nodes share-button-category" onclick="categoryShare(930)" style="position: absolute;top: -2px;right: -22px;font-size: 17px;background: #e6d5b7;padding: 7px 7px;border-radius: 50px;cursor: pointer;color: #000000;"></i>
                            @endif
                        </div>
                    @endforeach
                </div>
    ...
    <div class="col-xl-9 col-lg-8  mb-40">
        <div class="row custom-row product-gap-row row-gap-30px mb-100px">
            @foreach ($products as $product)
                <div class="col-lg-3 col-sm-6">
                    ...
                    <a class="d-flex justify-content-between flex-column"
                            ...
                            <div>
                                <div class="product-img">
                                    <img src="{{ $product->images_url[0] ?? '' }}" alt="product-img"
                                        class="h-100 w-100 object-fit-cover product-image" loading="lazy" />
                                </div>
                        ...
                        <div>
                            @if($whatsappStore->id != 171 && $whatsappStore->id != 322 && $whatsappStore->id != 187 && $whatsappStore->id != 1065)
                            <button
                        ...
                        <button
                            class="@if($product->available_stock == 0) disabled @endif btn btn-primary d-flex justify-content-center align-items-center w-100 mx-auto"
                            onclick="prepareAndSendWpMessageDirect({{ $product->id }},'{{ $product->name }}','{{ $product->currency->currency_icon }}','{{$product->selling_price}}')" style="background: #25d366 !important; color: #ffffff !important;border: 1px solid #25d366 !important;"
                            data-id="{{ $product->id }}">
                            <svg width="800px" height="800px" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg" style="height: 25px;width: 25px;">
                                                    <path fill-rule="evenodd" clip-rule="evenodd" d="M16 31C23.732 31 30 24.732 30 17C30 9.26801 23.732 3 16 3C8.26801 3 2 9.26801 2 17C2 19.5109 2.661 21.8674 3.81847 23.905L2 31L9.31486 29.3038C11.3014 30.3854 13.5789 31 16 31ZM16 28.8462C22.5425 28.8462 27.8462 23.5425 27.8462 17C27.8462 10.4576 22.5425 5.15385 16 5.15385C9.45755 5.15385 4.15385 10.4576 4.15385 17C4.15385 19.5261 4.9445 21.8675 6.29184 23.7902L5.23077 27.7692L9.27993 26.7569C11.1894 28.0746 13.5046 28.8462 16 28.8462Z" fill="#BFC8D0"></path>
                                                    <path d="M28 16C28 22.6274 22.6274 28 16 28C13.4722 28 11.1269 27.2184 9.19266 25.8837L5.09091 26.9091L6.16576 22.8784C4.80092 20.9307 4 18.5589 4 16C4 9.37258 9.37258 4 16 4C22.6274 4 28 9.37258 28 16Z" fill="url(#paint0_linear_87_7264)"></path>
            ...
            <script>
                function showFilter(){
                    if($("#filterButtonNew").attr("data-content") == "Show Filter"){
                        $("#filterButtonNew").attr("data-content","Hide filter");
                        $("#filterMenu").animate({ height: '600px' }, 300);
                        setTimeout(() => {
                            $("#filterMenu").css("height", "100%");    
                        }, 400);   
                    }else{
                        $("#filterButtonNew").attr("data-content","Show Filter");
                        $("#filterMenu").animate({ height: '76px' }, 300);
                    }
