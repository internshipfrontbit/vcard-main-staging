 ...
 <nav class="navbar navbar-expand-lg position-relative">
    ...
    <div class="main-content mx-auto w-100 overflow-hidden d-flex flex-column justify-content-between {{ getLocalLanguage() == 'ar' ? 'rtl' : '' }}" @if(getLanguage($whatsappStore->default_language) == 'Arabic') dir="rtl" @endif>
        ...
        <div>
                    ...
                    @endif
                        <div class="search-input-container position-relative mb-4" style="margin-right: 25px; margin-left: 23px; margin-top: 20px;">
                            <input type="text" class="form-control" placeholder="Enter product name" name="search">
                            <button type="submit" style="position: absolute;top: 1px;right: 1px;background: #72bf78;color: #000;padding: 6px 10px;border-radius: 4px;cursor: pointer;border: none !important;"><i class="fas fa-search" ></i></button>
                        </div>
                    </form>
            ...
            <div class="product-section px-4 px-4-custom pt-30 position-relative">
                    ...
                    @foreach ($whatsappStore->products()->where('is_active','=',true)->orderByRaw('position IS NULL, position ASC')->orderBy('created_at', 'desc')->take(8)->get() as $product)
                        <div class="col-xl-3 col-lg-4 col-sm-6 custom-margin-bottom mb-30">
                            ...
                            @endif
                                    ...
                                    <div class="product-img bg-yellow" style="position: relative;">
                                ...
                                <button data-id="{{ $product->id }}"
                                    class="@if($product->available_stock == 0) disabled @endif mb-2 btn btn-primary d-flex justify-content-center align-items-center mx-auto gap-2 add-to-cart-w-140px w-100 addToCartBtn">
                                    <span>
                                ...
                                @if($whatsappStore->id != "26")
                                    <button data-id="{{ $product->id }}"
                                    class="mb-2 btn btn-primary d-flex justify-content-center align-items-center mx-auto gap-2 add-to-cart-w-140px w-100" onclick="prepareAndSendWpMessageDirect({{ $product->id }},'{{ $product->name }}','{{ $product->currency->currency_icon }}','{{$product->selling_price}}')" style="background: #25d366 !important; color: #ffffff !important;border: 1px solid #25d366 !important;">
                                    <span>
        ...
        @include('whatsapp_stores.templates.grocery_store.attributes_model')
...
<!--</div>                -->
  ...
  @if($whatsappStore->id == 493)        
    <a id="whatsappGifNew" href="tel:+918487938602" style="position: fixed;right: 10px;bottom: 28px;z-index: 100;">
