    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.success) {
                ...
                $("#editProductCode").val(result.data.p_code);
                  $("#editAtributeTitle").val(result.data.atribute_title);
                $("#editProductBrand").val(result.data.p_brand);
...
listenChange(".product-order-payment-status", function () {
    let orderId = $(this).data("id");
    let status = $(this).val();
    if (status == 0) return;
    console.log("changes");
