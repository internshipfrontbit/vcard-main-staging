    ...
    public function store($input)
    {
        try {
            ...
            if ($input['type'] == Gallery::TYPE_IMAGE) {
                $imageCount = Gallery::where('vcard_id', $input['vcard_id'])
                    ->where('type', Gallery::TYPE_IMAGE)
                    ->count();
    
                if ($imageCount >= 20) {
                    throw new UnprocessableEntityHttpException('You can upload a maximum of 20 images per vCard.');
                }
