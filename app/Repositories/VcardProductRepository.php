...
class VcardProductRepository extends BaseRepository
{
    ...
    public function store($input)
    {
        try {
            ...
            $productCount = Product::where('vcard_id', $input['vcard_id'])->count();
            
            if ($productCount >= 20) {
                throw new UnprocessableEntityHttpException('You can upload a maximum of 20 products for this vCard.');
            }
            ...
            if (isset($input['product_icon']) && !empty($input['product_icon'])) {
                ...
                $firstImage = is_array($input['product_icon']) ? reset($input['product_icon']) : $input['product_icon'];
            
                $vcardProduct->newAddMedia($firstImage)->toMediaCollection(
