...
class WpStoreTemplatesProductsList extends Component
{
    ...
    public function updatingStockFilter() // Reset page on stock filter change
    {
        $this->resetPage();
    }
    ...
    {
        $this->reset(['search', 'dateFilter', 'minPrice', 'maxPrice', 'priceSortOrder', 'selectedCategory', 'categoryFilter','stockFilter']);
        $this->resetPage();
    }
    ...
    public function render()
    {
        ...
        if($this->whatsappStoreId == 208 || $this->whatsappStoreId == 1488){
            $productlimit = 100;
        }

        if($this->whatsappStoreId == 530){
            $productlimit = 50;
        }
