...
class SocialAuthController extends Controller
{
    ...
    public function handleSocialCallback($provider): \Illuminate\Http\RedirectResponse
    {
        ...
        $referrel_code = Cookie::get('referrel_code');
        $referral_user = '';
        if($referrel_code){
            $referral_user = User::where('affiliate_code', $referrel_code)->first();
        }
        ...
        try {
            ...
            if($activeVcard + $deActiveVcard != 0){
            return redirect(route('admin.dashboard'));
            }else{
                 return redirect(route('whatsapp.stores.store'));   
            }
        } catch (Exception $e) {
            DB::rollBack();
