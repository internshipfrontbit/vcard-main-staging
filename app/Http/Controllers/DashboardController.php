...
class DashboardController extends AppBaseController
{
    ...
    public function index(): \Illuminate\View\View
    {
        ...
        $totalWpTemplate = WpStoreTemplate::all()->count();
        
        $tenantId = auth()->user()->tenant_id;

        $totalOrder = WpOrder::whereHas('wpStore', function ($query) use ($tenantId) {
            $query->where('tenant_id', $tenantId);
        })->count();
        
        $totalPendingOrder = WpOrder::whereHas('wpStore', function ($query) use ($tenantId) {
    $query->where('tenant_id', $tenantId);
})->where('status', WpOrder::PENDING)->count();
